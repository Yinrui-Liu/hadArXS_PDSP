...
divided_trueEini, divided_weights = utils.divide_vars_by_partype(true_initial_energy, particle_type, mask=mask_combined, weight=reweight)
divided_trueEend, divided_weights = utils.divide_vars_by_partype(true_end_energy, particle_type, mask=mask_combined, weight=reweight)
divided_trueflag, divided_weights = utils.divide_vars_by_partype(true_sigflag, particle_type, mask=mask_combined, weight=reweight)
divided_trueisct, divided_weights = utils.divide_vars_by_partype(true_containing, particle_type, mask=mask_combined, weight=reweight)
...
XS_diff = XS_y - np.interp(XS_x, abs_simcurve[0], abs_simcurve[1]) # Need to fix this to not be hardcoded
...
plt.title("Exclusive XSec, Pion Inelastic")
